  ...
  try {
    ...
    if (validUrl.isUri(longurl)) {
      ...
      if (x) {
        ...
        let urlValue = await urlModel.findOne({ longUrl: req.body.longUrl }).select({ urlCode: 1, longUrl: 1, shortUrl: 1, _id: 0 })
        if (urlValue) {
...
//            return res.status(301).redirect(301, `${redirect1.longUrl}`);            
